...
- [ ] **F20** 파일 우클릭 → Search in Folder : 해당 폴더로 범위 제한된 전역 검색 열림

#### 폴더 우클릭 컨텍스트 메뉴

- [ ] **F21** 폴더 우클릭 → New File Here : 해당 폴더 내에 파일 생성 모달 열림
- [ ] **F22** 폴더 우클릭 → New Folder Here : 해당 폴더 내에 폴더 생성 모달 열림
- [ ] **F23** 폴더 우클릭 → Expand All : 모든 하위 폴더 펼침
- [ ] **F24** 폴더 우클릭 → Collapse All : 모든 하위 폴더 닫힘

#### 드래그 앤 드롭

- [ ] **F25** 파일을 다른 폴더로 드래그 : 파일이 해당 폴더로 이동됨
- [ ] **F26** 폴더를 다른 폴더로 드래그 : 폴더와 내부 파일 전체 이동됨
- [ ] **F27** 폴더를 자기 자신 안으로 드래그 : 이동 불가, 원위치 복귀
- [ ] **F28** 현재 열린 파일이 이동됐을 때 : 저장 상태 및 편집기 내용 유지

#### 외부 파일 변경 감시

- [ ] **F29** 외부 편집기로 현재 열린 파일 수정 : 외부 변경 감지 알림 표시
- [ ] **F30** 외부에서 새 파일 생성 : 파일 트리 자동 업데이트
- [ ] **F31** 외부에서 파일 삭제 : 파일 트리에서 자동 제거
...
- [ ] **U07** 저장 상태 표시 : unsaved / saving / saved / error 전환 올바름

#### 레이아웃

- [ ] **U08** 사이드바 리사이즈 드래그 : 사이드바 너비 조절 가능
- [ ] **U09** 전체 화면 전환 : 레이아웃 정상 유지
- [ ] **U10** 창 최소 크기 : 최소 크기 이하로 축소되지 않음

#### 코드 모드 (분할 뷰)

- [ ] **U11** `Cmd+2` 전환 : 왼쪽 마크다운 소스, 오른쪽 미리보기로 분할됨
- [ ] **U12** 소스 편집 : 미리보기 실시간 업데이트
- [ ] **U13** 코드 모드에서 `Cmd+1` : WYSIWYG 모드로 전환, 내용 유지

#### 자동 저장

- [ ] **U14** 타이핑 멈춘 후 5초 : 자동 저장 (saving → saved 전환)
- [ ] **U15** 타이핑 중 5초 미경과 : 저장 안 됨 (unsaved 유지)
- [ ] **U16** `Cmd+S` : 즉시 저장 (5초 대기 없음)
